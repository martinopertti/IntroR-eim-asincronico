...
# No ponderado con proporción
lat_17 %>% 
  group_by(int_voto_res) %>% # Variables de agrupar
  summarise(valor = n()) %>%  # Resumir frecuencias (ponderadas)
  mutate(per = valor / sum(valor)) # Agregar columna de porcentaje
...
lat_17 %>% 
  group_by(int_voto_res) %>% 
  summarise(frec_pond = sum(wt)) %>%  # Resumir frecuencias (ponderadas)
  mutate(per = paste0(round(frec_pond / sum(frec_pond) * 100, digits = 1), "%")) 
